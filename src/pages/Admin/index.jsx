...
function navBtn(active) {
  return {
    background: 'none',
    border: `1px solid ${active ? B.chartreuse + '44' : B.border}`,
    borderRadius: '10px', padding: '8px 16px',
    color: active ? B.cream : B.muted,
...
export default function AdminRoot() {
  ...
  const activeTab = path.startsWith('/admin/dashboard') ? 'dashboard'
    ...
    <div style={{ minHeight: '100vh', background: B.canvas, display: 'flex', flexDirection: 'column' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          <button onClick={() => navigate('/admin')} style={navBtn(activeTab === 'events')}>Events</button>
          <button onClick={() => navigate('/admin/dashboard')} style={navBtn(activeTab === 'dashboard')}>Dashboard</button>
          <button onClick={() => navigate('/admin/team')} style={navBtn(activeTab === 'team')}>Team</button>
          <div style={{ width: '1px', height: '16px', background: B.border }} />
